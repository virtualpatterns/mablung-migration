...
Test.serial('list', async (test) => {
  test.is(await (new LoggedForkedProcess(test.context.logPath, Path.resolve(FolderPath, '../../command/index.js'), { 'list': true }, { 'cwd': Path.resolve(FolderPath, 'no-migration') })).whenExit(), 1)
})
