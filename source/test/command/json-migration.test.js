...
Test('list', async (test) => {
  test.is(await (new LoggedForkedProcess(test.context.logPath, Path.resolve(FolderPath, '../../command/index.js'), { 'list': true }, { 'cwd': Path.resolve(FolderPath, 'json-migration') })).whenExit(), 0)
})
